...
def test_post_method_json(client):
    ...
    response = client.post(
        "/post", data=json.dumps(test_data), content_type="application/json"
    )
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_put_method(client):
    ...
    response = client.put(
        "/put", data=json.dumps(test_data), content_type="application/json"
    )
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_patch_method(client):
    ...
    response = client.patch(
        "/patch", data=json.dumps(test_data), content_type="application/json"
    )
    assert response.status_code == 200
    data = json.loads(response.data)
